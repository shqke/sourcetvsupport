...
bool CGameData::SetupFromGameConfig(IGameConfig* gc, char* error, int maxlength)
{
	...
	static const struct {
		...
		{ "CFrameSnapshotManager::LevelChanged", pfn_CFrameSnapshotManager_LevelChanged },
...
bool CGameData::SetupFromSteamAPILibrary(char* error, int maxlength)
{
	...
	if (!steam_api) {
		ke::SafeSprintf(error, maxlength, "Unable to load library \"%s\" (reason: \"%s\")", path, failReason);
